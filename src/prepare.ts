...
export function prepareUpxFiles(config: ResolvedConfig, localUrl?: string) {
  ...
  writeFileSync(getDistPath(config, 'plugin.json'), JSON.stringify(data, null, 2), { encoding: 'utf-8' })
  copyUpxFiles(config);
}
...
/**
 * @description 生产 preload 类型文件，与 preload 在同级目录
 * @param content 要写入的内容
 * @returns 文件名
 */
export function buildPreloadTsd(content: string) {
  const dstFileName = '_' + getPreloadId() + '.d.ts'
  writeFileSync(resolvePathToPreload(dstFileName), content, { flush: true })
  return dstFileName
}