...
export function analyzePreloadFile(
      ...
      if (node.importClause) {
        ...
        if (node.importClause.namedBindings) {
          ...
          if (ts.isNamespaceImport(node.importClause.namedBindings)) {
            allDeclarations[node.importClause.namedBindings.name.text] =
  ...
  function analyzeObjectLiteral(objNode: ts.ObjectLiteralExpression): Exports {
    ...
    objNode.properties.forEach((prop) => {
      ...
      if (ts.isMethodDeclaration(prop)) {
        ...
        if (
          ts.isFunctionExpression(valueNode) ||
          ts.isArrowFunction(valueNode)
        ) {
          objectInfo[key] = getFunctionInfo(valueNode);
        } else if (ts.isObjectLiteralExpression(valueNode)) {
          objectInfo[key] = {
            type: 'Object',
            props: analyzeObjectLiteral(valueNode),
          };
        } else {
   ...
   * 分析外部模块，返回其导出信息
   */
  function analyzeExternalModule(
    ...
    try {
      ...
      if (
        !externalFilePath.endsWith('.ts') &&
        !externalFilePath.endsWith('.js')
      ) {
        externalFilePath += '.ts';
      }
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `无法读取文件 ${externalFilePath}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
  ...
  function handleExportIdentifier(
    identifier: string,
    exportFromPath?: string,
    currentFileName?: string,
    ...
    if (exportFromPath) {
      const externalExports = analyzeExternalModule(
        exportFromPath,
        currentFileName,
      );
      if (externalExports) {
    ...
    if (ts.isVariableDeclaration(init)) {
      if (init.initializer) {
        if (
          ts.isArrowFunction(init.initializer) ||
          ts.isFunctionExpression(init.initializer)
        ) {
        ...
        if (ts.isIdentifier(init.initializer)) {
          return handleExportIdentifier(
            init.initializer.text,
            undefined,
            currentFileName,
          );
        }
    ...
    if (ts.isImportSpecifier(init)) {
      ...
      if (parent && ts.isImportDeclaration(parent)) {
        const moduleSpecifier = (parent.moduleSpecifier as ts.StringLiteral)
          .text;
        const propertyName = init.propertyName
          ? init.propertyName.text
          : init.name.text;
        return handleExportIdentifier(
          propertyName,
          moduleSpecifier,
  ...
  function visit(node: ts.Node, currentFileName?: string) {
            ...
            if (ts.isIdentifier(name)) {
              ...
              if (init) {
                ...
                if (ts.isArrowFunction(init) || ts.isFunctionExpression(init)) {
                  ...
                  exports.namedExports[nameText] = {
                    type: 'Object',
                    props: analyzeObjectLiteral(init),
                  };
                } else if (value !== null) {
                  exports.namedExports[nameText] = { type: 'Constant', value };
                } else {
                  const entity = handleExportIdentifier(
                    nameText,
                    undefined,
                    currentFile,
                  );
          ...
          if (moduleSpecifier) {
            ...
            if (externalExports) {
              ...
              if (externalExports.defaultExport) {
                nsExports['default'] = {
                  type: 'Object',
                  props: externalExports.defaultExport,
                };
              }
    ...
    if (ts.isExportAssignment(node)) {
      if (ts.isObjectLiteralExpression(node.expression)) {
        ...
        exports.defaultExport = {
          [name]: handleExportIdentifier(name, undefined, currentFile),
        } as Exports;
      } else if (
 ...
 */
export function generateAutoMockCode(
  preloadGlobalName: string,
  exportsInfo: AllExportsInfo,
) {
...
// 该文件由 @ver5/vite-plugin-utools 自动生成
// @ts-nocheck
 ...
 */
export function purgePreloadbundle(code: string): {
  code: string;
  ...
  // 生成不冲突的变量名，用于替换 CommonJS 导出语句
  // 例如： exports.someVar = 123;
  //       -> _up_someVar = 123;
  //       -> exports.someVar = _up_someVar;
  //       -> exportMap['someVar'] = '_up_someVar';
  //       -> return { code: '..._up_someVar...', exportMap };
  const generateSafeInfo = (name: string, exists: Set<string>) => {
    let newName = `_up_${name}`;
    let i = 1;
    while (exists.has(newName)) {
      newName = `_up_${name}_${i}`;
      i++;
    }
    return newName;
  };
  ...
  ts.forEachChild(sourceFile, (node) => {
    if (ts.isVariableStatement(node)) {
      node.declarationList.declarations.forEach((decl) => {
        if (ts.isIdentifier(decl.name)) {
          declaredVariables.add(decl.name.text);
        }
      });
    }
  });
  ...
  ts.forEachChild(sourceFile, (node) => {
    ...
    if (
      ...
      if (
        ...
        (expr.expression.name.text === 'defineProperties' ||
      ...
      if (
        ...
        if (name === 'default') {
          ...
          s.overwrite(
            ...
            node.end,
            `Object.assign(window, ${valueText})`,
          );
        } else {
          // Check for chained exports or void 0
          if (
            right.kind === ts.SyntaxKind.VoidExpression ||
            valueText === 'undefined'
          ) {
            s.remove(node.pos, node.end);
            return;
          ...
          if (name === valueText.trim()) {
            ...
            // 变量名冲突，使用别名
            const safeName = generateSafeInfo(name, declaredVariables);
            declaredVariables.add(safeName);
            exportMap[name] = safeName;
            s.overwrite(
              node.getStart(sourceFile),
              node.end,
              `const ${safeName} = ${valueText};`,
            );
          } else {
            // exports.hello = ... -> const hello = ...
            declaredVariables.add(name); // 标记为已占用
            s.overwrite(
              node.getStart(sourceFile),
              node.end,
...
/**
 * @description 构建 tsd 类型声明文件内容
 * @param {string} name window上的挂载名（例如 preload）
 */
export function fillPreloadTsd(name: string, hasDefaultExport: boolean) {
  const preloadId = getPreloadId();
  let typesContent = `// 请不要直接修改此文件，因为它会在每次构建时被覆盖。
