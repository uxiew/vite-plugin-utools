 ...
 */
export function purgePreloadbundle(code: string): { code: string, hasDefaultExport: boolean, exportMap: Record<string, string> } {
            ...
            if (expr.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
                ...
                if (name === 'default') {
                    ...
                    if (name === valueText.trim()) {
                        ...
                        s.overwrite(node.getStart(sourceFile), node.end, `const ${safeName} = ${valueText};`);
                    } else {
