...
import {
  ...
  mockWrapperScript,
  utoolsApiMockScript,
} from './mocks/helperMock';
import {
  getPreloadPath,
  resolvePathToPreload,
...
import {
  analyzePreloadFile,
  generateAutoMockCode,
  generateUserMockCode,
} from './preload_analyzer';
...
export function preloadMockPlugin(options: RequiredOptions): Plugin {
  ...
  return {
    ...
    configureServer(_server) {
      ...
      const scaffoldMock = () => {
        try {
          ...
          console.log(`[uTools Mock] 生成 mock 文件: ${preloadMockAutoPath}`);
          ...
          if (!existsSync(preloadMockPath)) {
            ...
            console.log(
              `[uTools Mock] 生成自定义 mock 文件: ${preloadMockPath}`,
            );
          }
        } catch (e: any) {
          rc.logger.info(`[uTools Mock] 生成 mock 文件失败: ${e.message}`);
        }
      };
    ...
    handleHotUpdate({ file, server }) {
      if (file === preloadPath) {
        console.log('[uTools Mock] Preload 文件发生变化，更新 mock 文件...');
        try {
          const realCode = readFileSync(preloadPath, 'utf-8');
          const exportsInfo = analyzePreloadFile(realCode, preloadPath);
          const autoMockCode = generateAutoMockCode(globalName, exportsInfo);
          writeFileSync(preloadMockAutoPath, autoMockCode);
          console.log(`[uTools Mock] 更新了 ${preloadMockAutoPath}`);

          // 触发全量刷新，因为 mock 文件变了，可能影响全局
          server.ws.send({ type: 'full-reload' });
        } catch (e: any) {
          console.error(`[uTools Mock] 更新 mock 文件失败: ${e.message}`);
        }
      }
    },
    resolveId(id) {
